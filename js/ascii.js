...
var ascii = (function() {
  function asciiFromCanvas(canvas, options) {
    ...
    for (var y = 0; y < canvasHeight; y += 2) { // every other row because letters are not square
      for (var x = 0; x < canvasWidth; x++) {
        ...
        /* invert the pixels when the colours are changed */
        if (app.asciiContainer.classList.contains('dark')){
          color = invertPixels(imageData.data, offset);
        }
        else{
  ...
  function invertPixels(data, offset){
    return{
      red: 225 - data[offset],
      green: 225 - data[offset + 1],
      blue: 225 - data[offset + 2],
      alpha: data[offset + 3]
    };
  }
